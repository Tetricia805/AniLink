...
| `['orders']` | Owner orders | createOrder, cancelOrder |
| `['seller-orders']` | Seller orders | createOrder, updateSellerOrderStatus, cancelOrder |
...
## Notification "View details" routing

- **Web:** `anilink-web/src/lib/notificationRouting.ts` — `getNotificationHref(n, role)` returns path or `undefined`. Used by `NotificationsPage`; if no href, user sees toast "This notification has no linked item."
- **Mobile:** `apps/mobile/src/lib/notificationRouting.ts` — `getNotificationDestination(n, role)` returns `{ type: 'navigate', screen, params }` or `{ type: 'noLink' }`. No link → Alert "This notification has no linked item."
- **Fallback:** Prefer `actionUrl` if present; else derive from `entityType` + `entityId` (and `relatedId`) by role. Supported patterns: bookings (`/appointments?focus=`, `/vet/appointments?focus=`), orders (`/orders/<id>`, `/seller/orders?focus=`), cases (`/records?focusCase=`, `/vet/cases?focus=`).

## Order detail actions

- **Web:** `anilink-web/src/pages/OrderDetailPage.tsx` — `useOrder(id)`, `useCancelOrder()`, `ContactSellerModal` (seller phone/email: tel:, mailto:, copy). Cancel invalidates `['orders']`, `['orders', id]`, `['seller-orders']`, `['notifications']`.
- **Mobile:** `apps/mobile/src/screens/owner/OrderDetailScreen.tsx` — `useOrder(orderId)`, `useCancelOrder()`, contact via `Linking.openURL('tel:...')` / `mailto:...`; if no seller phone/email, button disabled and "Seller contact not available" shown. Cancel shows Snackbar "Order cancelled" and invalidates same keys.
