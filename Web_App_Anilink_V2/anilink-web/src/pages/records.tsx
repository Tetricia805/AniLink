...
export function RecordsPage() {
  ...
  const { data: apiAnimals = [], isLoading: animalsLoading } = useAnimals();
  ...
  const handleCreateAnimal = (data: Parameters<typeof createAnimal.mutate>[0]) => {
  ...
  const handleStartScan = () => {
    if (selectedAnimal) {
      navigate(`/scan/start?animalId=${selectedAnimal.id}`);
    } else {
      navigate("/scan/start");
    }
  };
            ...
            {selectedAnimal ? (
              ...
              <Card>
                <CardContent className="py-16 text-center">
                  ...
                  <h3 className="text-lg font-medium mb-2">No animal selected</h3>
                  <p className="text-muted-foreground mb-6">
                    Add an animal or select one from the list to view health
                    records
                  </p>
                  <Button
                    type="button"
