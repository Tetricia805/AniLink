  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-6 md:py-8 max-w-5xl">
        <h1 className="text-2xl md:text-3xl font-semibold mb-2">Cases</h1>
        <p className="text-muted-foreground mb-6">
          Cases submitted by owners (e.g. from scan results). Add notes, recommend treatment, mark resolved.
        </p>
        ...
        {cases.length === 0 ? (
          <Card>
            <CardContent className="p-8">
              <EmptyState
                ...
                description="When owners submit scans or request help, cases will appear here once assigned to you."
              />
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-4">
            {cases.map((c) => (
              <Card
                key={c.id}
                className={focusedCase?.id === c.id ? "ring-2 ring-primary" : ""}
              >
