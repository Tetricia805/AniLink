...
export function useCreateSellerProduct() {
  ...
  return useMutation({
    ...
    onError: (error: unknown) => {
      const msg = (error as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Failed to add product'
...
export function useUpdateSellerProduct() {
  ...
  return useMutation({
    ...
    onError: (error: unknown) => {
      const msg = (error as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Failed to update'
      push({ title: 'Error', description: msg })
    },
  })
}
